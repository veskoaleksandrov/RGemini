...
# 
# In this example we are creating two applications
# First for a fictional company called Contoso
# Second for a fictional company called ACME
myapp <- list(contoso = oauth_app(appname = "yahoo", 
                               key = "", 
                               secret = "", 
                               redirect_uri = "oob"), 
              acme = oauth_app(appname = "yahoo", 
                             key = "", 
                             secret = "", 
                             redirect_uri = "oob"))

# keep authorization codes updated
authorizationCode <- list(contoso = "", 
                          acme = "")

refresh_token <- list(contoso = readRDS(file = "O:/Path/To/Your/Authorisation/Token/gemini-contoso.rds"), 
                      acme = readRDS(file = "O:/Path/To/Your/Authorisation/Token/gemini-acme.rds"))
...
for(t in 1:length(refresh_token)) {
  ...
  for(a in 1:nrow(advertiser)) {
    ...
    # Union Contoso and ACME reports
